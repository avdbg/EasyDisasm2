...
void x86asm::prepInsns()
{
	if (!x86_32a_insns)
	{
		x86_32a_insns = (x86opc_insn (*)[256])malloc(sizeof *x86_32a_insns);
...
asm_code *x86asm::encode(asm_insn *asm_insn, int _options, CPU_ADDR cur_address)
{
...
void x86asm::deletecode(asm_code *code)
{
...
void x86asm::clearcode()
{
	code.size = 0;
}

void x86asm::emitbyte(byte b)
{
	code.data[code.size] = b;
	code.size++;
}

void x86asm::emitword(uint16 w)
{
	if (bigendian)
	{
...
void x86asm::emitdword(uint32 d)
{
	if (bigendian)
	{
...
void x86asm::emitqword(uint64 q)
{
	if (bigendian)
	{
...
const char *x86asm::get_error_msg()
{
	return error_msg;
}
...
asm_code *x86asm::shortest(asm_code *codes)
{
	asm_code *best = NULL;
	int bestv = INT_MAX;
	while (codes)
...
void x86asm::set_error_msg(const char *format, ...)
{
	va_list arg;
	va_start(arg, format);
...
void x86asm::set_imm_eval_proc( int (*pfn)(void *context, const char *s, uint64 &v), void *pContext )
{
	imm_eval_proc = pfn;
	imm_eval_context = pContext;
}
